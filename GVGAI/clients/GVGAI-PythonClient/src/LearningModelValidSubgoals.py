...
# Model used to select if a subgoal is valid (the agent won't die nor end up in a dead end)
# Given a pair (observation, subgoal) encoded as a one-hot observation matrix
# the model predicts the probability of the agent dying or not being able to complete the level
# (being in a dead end) after achieving the subgoal
# Probability 1 -> the agent should NEVER select this subgoal
# Probability 0 -> the subgoal is valid (can be selected by the agent)

class DQNetworkValidSubgoals:
    ...
    # Predicts the associated y-value (probability of valid subgoal) for x (a (subgoal, game state) pair one-hot encoded)
    def predict(self, x):
    ...
    # Predicts the associated y-value (probability of valid subgoal) for a batch of x (a (subgoal, game state) pair one-hot encoded)
    def predict_batch(self, x):
        data_dict = {self.X : x, self.is_training : False}
    ...
    # every tau steps
    # update_ops is a list of tensorflow operations to be executed
