...
class DQNetwork:
	...
	def __init__(self, name="DQNetwork", writer_name="DQNetwork", create_writer = True,
		...
		with tf.variable_scope(self.variable_scope):
			...
			# <Concatenate output of last conv layer with inputs>

			# Flatten output of conv layers
			self.flatten_conv = tf.contrib.layers.flatten(self.conv6)

			# Flatten the input (one-hot matrix)
			self.flatten_input = tf.contrib.layers.flatten(self.X_norm)

			# Concatenate inputs to conv layers outupt
			self.concatenate = tf.concat([self.flatten_input, self.flatten_conv], 1)


			# Fully connected layer 1

			self.fc_1 = tf.layers.dense(inputs = self.concatenate,
