...
# of a model as the number of training iterations increases
...
for curr_match in match_set:
	...
	# List of lists of the number of actions
	# Group 1 (of this new match) -> list with number of actions
	...
	# Calculate the actions for each level for each element in curr_model_match_list_num_actions
	...
	# Calculate the normalized average number of actions for each different number of its
	...
	results_list = sorted(zip(its_list, norm_averaged_num_actions))

	# Print the number of iterations and average number of actions per each num its
	results_list_its = [i[0] for i in results_list]
	results_list_num_act = [i[1] for i in results_list]

	print("\n----- {} - {} -----".format(curr_match[0], curr_match[1]))
	print("> Num its: ", results_list_its)
	print("> Average actions: ", results_list_num_act)