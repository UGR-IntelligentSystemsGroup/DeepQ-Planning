...
class DQNetwork:
	...
	def save_logs(self, X, Orientation, A, Y, it):
		...
		train_loss_log, Q_val_log, Q_target_log, action_up_log, action_right_log, action_down_log, action_left_log, \
		 ...
		 self.action_use_sum, self.orient_up_sum, self.orient_right_sum, self.orient_down_sum, self.orient_left_sum],
		 feed_dict=data_dict_train)

		self.writer.add_summary(train_loss_log, it)
		self.writer.add_summary(Q_val_log, it)
		self.writer.add_summary(Q_target_log, it)
		self.writer.add_summary(action_up_log, it)
		self.writer.add_summary(action_right_log, it)
		self.writer.add_summary(action_down_log, it)
		self.writer.add_summary(action_left_log, it)
		self.writer.add_summary(action_use_log, it)
